<template>
    ...
    <BrandForm ref="refBrandForm" @hideComp="hideComp" @selectPageReq="selectPageReq"/>
</template>
...
<script setup>
    ...
    let tableEditClick = (row) => {
        refBrandForm.value.showModal(true, row)
    }
    /*3.详情modal*/
    let detailData = ref({})
    let tableDetailClick = (row) => {
        proxy.dialogTitleMixin = `详情【${row.username}】`
        refBrandForm.value.getDetailByIdReq(row.id).then((resData) => {
